...
def pc_setup(data):
    ...
    elections['MARGIN %'] = elections[
        'MARGIN'] / elections['VOTES'].astype(float)
    ...
    elections['WOMEN %'] = elections['WOMEN'] / \
        elections['CANDIDATES'].astype(float)
...
def latlong_setup(latlong):
    ...
    for state, indices in latlong.groupby('ST_NAME').groups.iteritems():
        xy = layout.unpack(
            latlong.ix[indices][['LONG', 'LAT']],
            [R] * len(indices), gravity=0, factor=.99)
